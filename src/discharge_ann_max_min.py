...
for i in np.arange(lexp):
    ...
    ax1.plot(ma.masked_less_equal(org_max[i],0.0).flatten(),ma.masked_where(org_max[i]<=0.0,asm_max[i]).flatten(),marker="o",fillstyle="none",markersize=3.0,linestyle="none",linewidth=0,color=colorsA[i],zorder=112)
    #===============
    slope, intercept, r, p, se = stats.linregress(ma.masked_less_equal(org_max[i],0.0).flatten(), ma.masked_where(org_max[i]<=0.0,asm_max[i]).flatten())
    datamax=max(np.max(ma.masked_less_equal(org_max[i],0.0).flatten()),np.max(ma.masked_where(org_max[i]<=0.0,asm_max[i]).flatten()))
    datamin=min(np.min(ma.masked_less_equal(org_max[i],0.0).flatten()),np.min(ma.masked_where(org_max[i]<=0.0,asm_max[i]).flatten()))
    ...
    r_asm=np.corrcoef(ma.masked_less_equal(org_max[i],0.0).flatten(), ma.masked_where(org_max[i]<=0.0,asm_max[i]).flatten())[0,1]
    s_asm=slope
    print r_asm, s_asm
    # ax1.text(0.8,0.2,r_sqr,ha="left",va="center",transform=ax1.transAxes,fontsize=6)

    # Open
    ax1.plot(ma.masked_less_equal(org_max[i],0.0).flatten(),ma.masked_where(org_max[i]<=0.0,opn_max[i]).flatten(),marker="s",fillstyle="none",markersize=3.0,linestyle="none",linewidth=0,color=colorsO[i],zorder=111)
    #===============
    slope, intercept, r, p, se = stats.linregress(ma.masked_less_equal(org_max[i],0.0).flatten(), ma.masked_where(org_max[i]<=0.0,opn_max[i]).flatten())
    datamax=max(np.max(ma.masked_less_equal(org_max[i],0.0).flatten()),np.max(ma.masked_where(org_max[i]<=0.0,asm_max[i]).flatten()))
    datamin=min(np.min(ma.masked_less_equal(org_max[i],0.0).flatten()),np.min(ma.masked_where(org_max[i]<=0.0,asm_max[i]).flatten()))
    ...
    r_opn=np.corrcoef(ma.masked_less_equal(org_max[i],0.0).flatten(), ma.masked_where(org_max[i]<=0.0,opn_max[i]).flatten())[0,1]
    s_opn=slope
...
ax1.plot([0.0,260000.0],[0.0,260000.0],linestyle="--",linewidth=1.2,color="grey",zorder=110)
...
for i in np.arange(lexp):
    ...
    ax2.plot(ma.masked_less_equal(org_min[i],0.0).flatten(),ma.masked_where(org_min[i]<=0.0,asm_min[i]).flatten(),marker="o",fillstyle="none",markersize=3.0,linestyle="none",linewidth=0,color=colorsA[i],zorder=112)
    #===============
    slope, intercept, r, p, se = stats.linregress(ma.masked_less_equal(org_min[i],0.0).flatten(), ma.masked_where(org_min[i]<=0.0,asm_min[i]).flatten())
    datamax=max(np.max(ma.masked_less_equal(org_min[i],0.0).flatten()),np.max(ma.masked_where(org_min[i]<=0.0,asm_min[i]).flatten()))
    datamin=min(np.min(ma.masked_less_equal(org_min[i],0.0).flatten()),np.min(ma.masked_where(org_min[i]<=0.0,asm_min[i]).flatten()))
    ...
    r_sqr="$r^2 (assim)$: %3.2f"%(r)
    r_asm=np.corrcoef(ma.masked_less_equal(org_min[i],0.0).flatten(), ma.masked_where(org_min[i]<=0.0,asm_min[i]).flatten())[0,1]
    s_asm=slope
    # ax2.text(0.8,0.2,r_sqr,ha="left",va="center",transform=ax2.transAxes,fontsize=6)
        
    # Open
    ax2.plot(ma.masked_less_equal(org_min[i],0.0).flatten(),ma.masked_where(org_min[i]<=0.0,opn_min[i]).flatten(),marker="s",fillstyle="none",markersize=3.0,linestyle="none",linewidth=0,color=colorsO[i],zorder=111)
    #===============
    slope, intercept, r, p, se = stats.linregress(ma.masked_less_equal(org_min[i],0.0).flatten(), ma.masked_where(org_min[i]<=0.0,opn_min[i]).flatten())
    datamax=max(np.max(ma.masked_less_equal(org_min[i],0.0).flatten()),np.max(ma.masked_where(org_min[i]<=0.0,asm_min[i]).flatten()))
    datamin=min(np.min(ma.masked_less_equal(org_min[i],0.0).flatten()),np.min(ma.masked_where(org_min[i]<=0.0,asm_min[i]).flatten()))
    ...
    r_sqr="$r^2 (open)$: %3.2f"%(r)
    r_opn=np.corrcoef(ma.masked_less_equal(org_min[i],0.0).flatten(), ma.masked_where(org_min[i]<=0.0,opn_min[i]).flatten())[0,1]
    s_opn=slope
...
ax2.plot([0.0,130000.0],[0.0,130000.0],linestyle="--",linewidth=1.2,color="grey",zorder=110)
