...
def plot_histogram(lstato1, lstato2, color1, color2, numch, ylabel, xlabel,ax=None,left=False,bottom=False):
    ...
    ax=ax or plt.gca()
    #
    sns.distplot(lstato1,ax=ax, hist = True, kde = True,
            kde_kws = {'linewidth': 1,'linestyle':'-'},
            label = "Uncalibrated", color=color1,norm_hist=False)
    # median line
    ax.axvline(x=np.median(np.array(lstato1)), linestyle="-", color="k")
    sns.distplot(lstato2,ax=ax, hist = True, kde = True,
            kde_kws = {'linewidth': 1,'linestyle':'--'},
            label = "Calibrated", color=color2,norm_hist=False)
    # median line
    ax.axvline(x=np.median(np.array(lstato2)), linestyle="--", color="k")
    if left:
        ax.set_ylabel(ylabel,fontsize=8)
    if bottom:
...
def ret_metric(exp1,exp2):
    ...
    for line in lines[1::]:
        ...
        try:
            ...
            asmW2, opnW2 = read_wse(exp2,VSstation,"simulation")
        except:
            asmW1, opnW1 = read_wse(exp1,VSstation,"validation")
            asmW2, opnW2 = read_wse(exp2,VSstation,"validation")
        #--------------
        ...
        rmseo1=RMSE(opnW1,orgW1)
        rmseo2=RMSE(opnW2,orgW1)
...
# colors1=["#004488","#ddaa33","#ba5566"]
# colors2=["#6699cc","#edcc65","#ee99aa"]

colors1=["#D81B60","#FFC107","#004D40"]
colors2=["#F382AB","#FFEDB9","#04B597"]
...
for j in np.arange(3):
    ...
    # plot_histogram(lNSEAI1,lNSEAI2, colors1[j],colors2[j], titles[j],"Number of Gauges","$NSEAI$",ax=axes[j],left=lefts[j],bottom=True)
