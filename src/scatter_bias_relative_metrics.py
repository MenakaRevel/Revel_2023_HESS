...
figname="figs14-bias_relative_metrics"
#===================
# argv=sys.argv
# syear=int(argv[1])
# eyear=int(argv[2])
...
def ret_metric(exp):
    ...
    for line in lines[1::]:
        ...
        print VSstation,"BIAS(a):",biasa, "BIAS(o):",biaso
        #--
        ldelr.append(DCOR)
        lNSEAI.append(NAI)
        lriss.append(rISS)
    ...
    return ldelr, lNSEAI, lriss, lbiaso, lbiasa
...
for j,exp in enumerate(lexp):
    ldelr, lNSEAI, lriss, lbiaso, lbiasa=ret_metric(exp)
    plot_scatter(ldelr, lbiaso, lbiasa,colors[j],"$\Delta$$r$","BIAS",ax1=ax1,ax2=ax2)
    plot_scatter(lNSEAI, lbiaso, lbiasa,colors[j],"$NSEAI$","BIAS",ax1=ax3,ax2=ax4)
    plot_scatter(lriss, lbiaso, lbiasa,colors[j],"$rISS$","BIAS",ax1=ax5,ax2=ax6)
