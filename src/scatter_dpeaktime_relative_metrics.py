...
#====================================================================
def plot_scatter(stat, lstato, lstata, color, ylabel, xlabel,ax1=None,ax2=None):
    ax1=ax1 or plt.gca()
    ax2=ax2 or plt.gca()
...
figname="figs15-dpeak_relative_metrics"
#===================
# argv=sys.argv
# syear=int(argv[1])
# eyear=int(argv[2])
...
def ret_metric(exp):
    ...
    for line in lines[1::]:
        ...
        print VSstation,"dpeak_time(a):",dpeaka, "dpeak_time(o):",dpeako
        #--
        ldelr.append(DCOR)
        lNSEAI.append(NAI)
        lriss.append(rISS)
    ...
    return ldelr, lNSEAI, lriss, ldpeako, ldpeaka
...
for j,exp in enumerate(lexp):
    ldelr, lNSEAI, lriss, ldpeako, ldpeaka=ret_metric(exp)
    plot_scatter(ldelr, ldpeako, ldpeaka,colors[j],"$\Delta$$r$","Peak\tTime\tDifference",ax1=ax1,ax2=ax2)
    plot_scatter(lNSEAI, ldpeako, ldpeaka,colors[j],"$NSEAI$","Peak\tTime\tDifference",ax1=ax3,ax2=ax4)
    plot_scatter(lriss, ldpeako, ldpeaka,colors[j],"$rISS$","Peak\tTime\tDifference",ax1=ax5,ax2=ax6)
