...
norm=BoundaryNorm(np.arange(vmin,vmax+0.1,0.2),cmap.N)
    ...
    for j,stat in enumerate(statistics):
        ...
        for line in lines[1::]:
            ...
            if stat=="r":
                ...
                vmin=-0.2
                vmax=0.2
                val1=correlation(asm1,org)
                val2=correlation(asm2,org)
                norm=BoundaryNorm(np.arange(vmin,vmax+0.1,0.05),cmap.N)
            elif stat=="NSE":
                # cmap=cm.get_cmap("RdBU_r")
                cmap=mbar.diverging_colormap('#052f61','#67001f')
                vmin=-0.5
                vmax=0.5
                val1=NS(asm1,org)
                val2=NS(asm2,org)
                ...
                val2=KGE(asm2,org)
                norm=BoundaryNorm(np.arange(vmin,vmax+0.1,0.1),cmap.N)
            metric_frag.append(val2-val1)
            #lon=lon0+ix*gsize
            #lat=lat0-iy*gsize
            lon=lonlat[0,iy1-1,ix1-1]
            lat=lonlat[1,iy1-1,ix1-1]
            c=cmap(norm(val2-val1))
        ...
        if j==0:
            ...
            plt.gca().text(-0.05,0.60,"%s-%s"%(labels[i2],labels[i1]),ha="center",va="center",transform=plt.gca().transAxes,fontsize=10,rotation=90)
        metric1.append(metric_frag)
    metric.append(metric1)

#======================
# cmap1=mbar.colormap("H02")
cmap1=cm.get_cmap("bwr_r")
vmin1=-0.2
vmax1=0.2
norm1=BoundaryNorm(np.arange(vmin1,vmax1+0.1,0.05),cmap1.N)
...
cbar1=plt.colorbar(im1,cax=cax1,ticks=np.arange(vmin1,vmax1+0.1,0.1),orientation='horizontal',extend="max")#np.arange(-0.5,0.5+0.001,0.25)
...
cmap2=mbar.diverging_colormap('#052f61','#67001f')
vmin2=-0.5
vmax2=0.5
...
l1,b1,w1,h1 = ax[-2].get_position().bounds
l2,b2,w2,h2 = ax[-1].get_position().bounds
cax2=fig.add_axes([l1+0.1*(w1+w2),b-0.1*(h1+h2),0.8*(w1+w2),0.01])
