...
def hydroweb_river_name(mapname="glb_15min"):
    ...
    return river
################################
def get_hydroweb(fname="/cluster/data6/menaka/HydroWeb/HydroWeb_alloc_glb_15min.txt",mapname="glb_15min"):
    # directory
...
################################
def get_hydroweb0(station0,fname="/cluster/data6/menaka/HydroWeb/HydroWeb_alloc_glb_15min.txt"):
    #--
    f=open(fname,"r")
    lines=f.readlines()
    for line in lines[1::]:
...
def HydroWeb_WSE(station,syear,eyear,smon=1,emon=12,sday=1,eday=31):
    ...
    return time, data
#####################################
def HydroWeb_continous_WSE(station,syear=2003,smon=1,sday=1,eyear=2020,emon=12,eday=31,egm08=0.0,egm96=0.0):
    #
    start=datetime.date(syear,smon,sday)
    end=datetime.date(eyear,emon,eday)
    # read hydroweb
