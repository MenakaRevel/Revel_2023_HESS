...
for i,exp in enumerate(labels):
    ...
    if exppre=="CaMa":
        ...
        for j,num in enumerate(nums):
            ...
            KGEasm=stat.KGE(dis_cmf.T[:,j],org)
            # if j==0:
            #     print "========================="
            #     for xx in np.arange(len(org)):
            #         print org[xx], dis_cmf.T[xx,j]
            # print num, NSEasm
            metric_frag.append(KGEasm)
    else:
        metric_frag=[]
        experiment=indirs[i]
        # print len(nums)
...
if seaborn_map==False:
    ...
    ax1.set_ylabel('$KGE$', color='k',fontsize=8)
...
#ax.set_title("BoxPlot NSEAI Rivers",fontsize=9,loc="left")
    ...
    ax1.set_ylabel('$KGE$', color='k',fontsize=12)
...
for i,exp in enumerate(labels):
    sns.distplot(metric[i,:],ax=ax2, hist = False, kde = True,
            kde_kws = {'linewidth': 1,'linestyle':'-'},
            label = labels[i], color=colors[i],norm_hist=False)
ax2.set_xlabel('$KEG$', color='k',fontsize=12) #$correlation$ $coefficient$ $(r)$
...
#======================
print (outdir+"/"+figname+".png")
#======================
plt.savefig(outdir+"/"+figname+".pdf",dpi=800,bbox_inches="tight", pad_inches=0.0)
plt.savefig(outdir+"/"+figname+".png",dpi=800,bbox_inches="tight", pad_inches=0.0)
plt.savefig(outdir+"/"+figname+".jpg",dpi=800,bbox_inches="tight", pad_inches=0.0)
