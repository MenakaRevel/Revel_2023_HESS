...
for i,exp in enumerate(experiments):
    ...
    for line in lines[1::]:
        ...
        metric.append([DCOR,NSEAI,KGEAI,rISS,rshr,rrel])
        if i==0:
            nums.append(num)
        # print (DCOR)
    ...
    # print experiments
    # print metric
    df=pd.DataFrame(data=metric, columns=["delta r","NSEAI","KGEAI","rISS","Sharpness","Reliability"], index=nums)
    df.index.name="GRDC ID"
    # print(df.head())
    df.to_csv(path_or_buf="./tables/realtive_metrics_"+exp+".csv")
    # calulation for delta_r
    print ("=======================")
    print (exp, "delta r")
    print (df["delta r"].count())
    # print (df["delta r"][df["delta r"]<-0.01].count())
    print ("Negative : ", df["delta r"][df["delta r"]<-0.01].count())
    print ("Zero : ", df["delta r"][abs(df["delta r"])<0.01].count())
    print ("Positve : ", df["delta r"][df["delta r"]>0.01].count())

    print ("Negative : ", df["delta r"][df["delta r"]<=0.0].count())
    print ("Positve : ", df["delta r"][df["delta r"]>0.0].count())