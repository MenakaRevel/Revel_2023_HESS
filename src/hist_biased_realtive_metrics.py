...
def plot_scatter(stat1, stat2, lstato1, lstato2, color1, color2, numch, ylabel, xlabel,ax=None,left=False,bottom=False):
    ...
    return 0
#====================================================================
def plot_histogram(lstato, color, numch, ylabel, xlabel,ax=None,left=False,bottom=False):
    # plot histogram using seaborn
    ax=ax or plt.gca()
    #
    sns.distplot(lstato,ax=ax, bins=10, hist = False, kde = True,
            kde_kws = {'linewidth': 0.5,'linestyle':'-'},
            color=color,norm_hist=True)
    # median line
    ax.plot(np.median(np.array(lstato)),0.01, marker="^", linestyle="none", color=color, linewidth=0.0)
    if left:
        ax.set_ylabel(ylabel,fontsize=8)
    if bottom:
    ...
    # ax.set_ylim(ymin=0.0 ,ymax=ymax+0.1*ymax)
    # ax.text(0.0,1.05,"%s"%(numch),ha="left",va="center",transform=ax.transAxes,fontsize=10)
    return 0
#====================================================================
...
texts=[]
axes=[ax1,ax2,ax3]
titles=["a) $\Delta$$r$", "b) $NSEAI$", "c) $rISS$"]
lefts=[True, False, False]
for j,exp in enumerate(lexp):
    ldelr, lNSEAI, lriss=ret_metric(exp)
    # direct DA
    plot_histogram(ldelr, colors[j], titles[j],"Probability Density","$\Delta$$r$",ax=ax1,left=True,bottom=True)
    plot_histogram(lNSEAI, colors[j], titles[j],"Probability Density","$NSEAI$",ax=ax2,left=False,bottom=True)
    plot_histogram(lriss, colors[j], titles[j],"Probability Density","$rISS$",ax=ax3,left=False,bottom=True)
#==============================
ax1.text(0.0,1.05,"a) $\Delta$$r$",ha="left",va="center",transform=ax1.transAxes,fontsize=10)
ax2.text(0.0,1.05,"b) $NSEAI$",ha="left",va="center",transform=ax2.transAxes,fontsize=10)
ax3.text(0.0,1.05,"c) $rISS$",ha="left",va="center",transform=ax3.transAxes,fontsize=10)
ymin, ymax=ax1.get_ylim()
ax1.set_ylim(ymin=0.0 ,ymax=ymax+0.01*ymax)
ymin, ymax=ax2.get_ylim()
ax2.set_ylim(ymin=0.0 ,ymax=ymax+0.01*ymax)
ax2.set_xlim(xmin=-10.2,xmax=1.2)
ymin, ymax=ax3.get_ylim()
ax3.set_ylim(ymin=0.0 ,ymax=ymax+0.01*ymax)
#==============================
patches=[]
