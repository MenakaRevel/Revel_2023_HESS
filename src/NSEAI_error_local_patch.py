...
def mk_fig(ldis,lwse,syear,eyear,metric,chtitle):
    ...
    ax0.text(-0.5,1.2,chtitle,transform=ax0.transAxes,fontsize=14)
    ax0.axis('off')
    tab=metric
...
for exp in lexp:
    ...
    lampdW[exp]=[]
#==============================
thr=0.05 # diffrece of mean river discharge for selecting discharge gauge
#
metric_list=[]
lNSEAI_list=[]
for ixiy in dislist.keys():
    #print dislist[ixiy]
    wsekey=ixiy
    diskey=ixiy
    #==================================
    ...
    if lQ == 0.0:
        continue
    #=====================================
    # reach here if obseration are avilable
    #=====================================
    ...
    for exp in lexp:
        ...
        #===============
        NSEasm=NS(asm,disobs)
        NSEopn=NS(opn,disobs)
        NSEAI=(NSEasm-NSEopn)/(1.0-NSEopn+1.0e-20)
        lNSEAI_frg.append(NSEAI)
    # lNSEAI_list.append(lNSEAI_frg)
    #=====================================
    ix=int(ixiy[0:5])
    iy=int(ixiy[5::])
    # open local patch file
    RMSEopn=[]
    Biasopn=[]
    Ampdopn=[]
    flag=0
    lW=0.0
    fname="/cluster/data6/menaka/Empirical_LocalPatch/local_patch/amz_06min_S14FD_60/patch%04d%04d.txt"%(ix,iy)
    with open(fname,"r") as f:
        lines=f.readlines()
    for line in lines:
        line    = re.split(" ",line)
        ...
        wgt     = float(line[2])
        #=====
        tmp_wsekey="%05d%05d"%(ixx,iyy)
        if tmp_wsekey in wselist.keys():
            snum=len(wselist[tmp_wsekey])
            for i in np.arange(snum):
                VS=wselist[tmp_wsekey][i]
                sfcobs=hweb.HydroWeb_continous_WSE(VS,syear=syear,eyear=eyear,egm08=lEGM08[VS],egm96=lEGM96[VS])
                if np.sum((sfcobs!=-9999.0)*1.0) <= 0.0:
                    continue
                lW=np.sum((sfcobs!=-9999.0)*1.0)
                # print (lW)
                # WSE
                try:
                    asm, opn = read_wse(lexp[0],VS,"simulation")
                except:
                    try:
                        asm, opn = read_wse(lexp[0],VS,"validation")
                    except:
                        print ("no file", exp, VS)
                        flag=-9
                # print (RMSE(opn,sfcobs), BIAS(opn,sfcobs), abs(dAmplitude(opn,sfcobs)))
                RMSEopn.append(RMSE(opn,sfcobs))
                Biasopn.append(BIAS(opn,sfcobs))
                Ampdopn.append(abs(dAmplitude(opn,sfcobs)))
    #===================
    # convert to array
    RMSEopn=np.array(RMSEopn)
    Biasopn=np.array(Biasopn)
    Ampdopn=np.array(Ampdopn)
    #
    if lW==0.0 or flag==-9:
        continue
    #===================
    # get the statistics
    # max_RMSE | median_RMSE | max_Bias | median_Bias | max_AmpD | median_AmpD
    metric_list.append([
        np.max(RMSEopn),np.median(RMSEopn),
        np.max(Biasopn),np.median(Biasopn),
        np.max(Ampdopn),np.median(Ampdopn) ])
    # NSEAI_DIR | NSEAI_ANO | NSEAI_NOM
    lNSEAI_list.append(lNSEAI_frg)
# print (metric_list)
#==========
# make dataframe
df1 = pd.DataFrame(lNSEAI_list, 
columns=["DIR","ANO","NOM"])
# print (df1.head())
df2 = pd.DataFrame(metric_list, 
columns=['max_RMSE','median_RMSE',
'max_Bias','median_Bias',
'max_AmpD','median_AmpD'])
# print (df2.head())
#---
...
# df = pd.concat([df1,df2.set_axis(df1.index)],axis=1)
...
#         print ("==============================================================")
#         print ("no common data",dislist[diskey], wselist[wsekey])
#         continue
#     flag=1
#     # print (sfcobs, disobs)
#     # observed values
...
#         NSEAI=(NSEasm-NSEopn)/(1.0-NSEopn+1.0e-20)
#         # print ("NSEAI", NSEAI)
#         lNSEAI[exp].append(np.nan_to_num(NSEAI))
#         lbiasQ[exp].append(np.nan_to_num((pBIAS(opn,orgd))))
#         lampdQ[exp].append(np.nan_to_num(abs(rAmplitude(opn,orgd))))
#         #
#         Q_list.append(asm)
#         if exp == lexp[-1]:
#             Q_list.append(opn)
...
#         # print ("rRMSE", rRMSE)
#         lRMSE[exp].append(np.nan_to_num(rRMSE))
#         lbiasW[exp].append(np.nan_to_num(BIAS(opn,orgw)))
#         lampdW[exp].append(np.nan_to_num(abs(dAmplitude(opn,orgw))))
#     if flag==-9:
#         continue
#     print ("==============================================================")
...
#plt.title(pname[point][0],fontsize=12)
G   = gridspec.GridSpec(3,2)
metlist=['max_RMSE','median_RMSE','max_Bias','median_Bias','max_AmpD','median_AmpD']
# ax1 = fig.add_subplot(G[0,0])
# ax2 = fig.add_subplot(G[0,1])
# ax3 = fig.add_subplot(G[1,0])
# ax4 = fig.add_subplot(G[1,1])
#============================
colors={lexp[0][0:3]:"#D81B60",lexp[1][0:3]:"#FFC107",lexp[2][0:3]:"#004D40"}
#============================
axlist=[]
for exp0 in lexp:
    exp=exp0[0:3]
    for i,met in enumerate(metlist):
        y=i/2
        x=i%2
        print (i, x, y)
        axlist.append(fig.add_subplot(G[y,x]))
        # ax=plt.gca()
        axlist[i].plot(df[met],df[exp],color=colors[exp],linestyle='None',linewidth=0,marker="o",fillstyle="none",markersize=5)
        slope, intercept, r, p, se = stats.linregress(df[met],df[exp])
        axlist[i].plot(np.sort(df[met]),intercept + slope*np.sort(df[met]), color=colors[exp])
        print (met, exp, slope, intercept, r, p, se)
#========================================================
axlist[0].text(-0.30,0.5,"RMSE",horizontalalignment='center',
        verticalalignment='center',rotation=90,transform=axlist[0].transAxes,fontsize=14)
axlist[0].text(0.5,1.2,"Max",horizontalalignment='center',
        verticalalignment='center',rotation=0,transform=axlist[0].transAxes,fontsize=14)
axlist[0].set_xlabel("$RMSE$ (open-loop)", color='k',fontsize=10)
axlist[0].set_ylabel("$NSEAI$", color='k',fontsize=10)
axlist[1].text(0.5,1.2,"Median",horizontalalignment='center',
        verticalalignment='center',rotation=0,transform=axlist[1].transAxes,fontsize=14)
axlist[1].set_xlabel("$RMSE$ (open-loop)", color='k',fontsize=10)
axlist[1].set_ylabel("", color='k',fontsize=10)
axlist[2].text(-0.30,0.5,"Bias",horizontalalignment='center',
        verticalalignment='center',rotation=90,transform=axlist[2].transAxes,fontsize=14)
axlist[2].set_xlabel("$Bias$ (open-loop)", color='k',fontsize=10)
axlist[2].set_ylabel("$NSEAI$", color='k',fontsize=10)
axlist[3].set_xlabel("$Bias$ (open-loop)", color='k',fontsize=10)
axlist[3].set_ylabel("", color='k',fontsize=10)
axlist[4].text(-0.30,0.5,"Amp Diff",horizontalalignment='center',
        verticalalignment='center',rotation=90,transform=axlist[4].transAxes,fontsize=14)
axlist[4].set_xlabel("$\Delta$ $AmpDiff$ (open-loop)", color='k',fontsize=10)
axlist[4].set_ylabel("$NSEAI$", color='k',fontsize=10)
axlist[5].set_xlabel("$\Delta$ $AmpDiff$ (open-loop)", color='k',fontsize=10)
axlist[5].set_ylabel("", color='k',fontsize=10)
# plt.show()
##########
plt.savefig("./pptimage/fig-NSEAI_local_patch_WSE_Q_error.jpg")  # saves the current figure into a pdf page
plt.close()
...
# vmin=-1.0
# vmax=1.0
# norm=BoundaryNorm(np.arange(vmin,vmax+0.1,0.05),cmap.N)
...
# axlist={}
# axlist[lexp[0]] = fig.add_subplot(G[0,0])
# axlist[lexp[1]] = fig.add_subplot(G[0,1])
# axlist[lexp[2]] = fig.add_subplot(G[0,2])
# #
# i=0
# for exp in lexp:
#     # X, Y = np.meshgrid(lbiasW[exp],lampdW[exp])
#     axlist[exp].scatter(lbiasW[exp], lampdW[exp], c=lNSEAI[exp],cmap=cmap,norm=norm)
#     axlist[exp].set_xlabel("$Bias$ (open-loop)", color='k',fontsize=10)
#     axlist[exp].text(0.01,1.1,string.ascii_lowercase[i]+") "+exp[0:3],horizontalalignment='left',
#         verticalalignment='center',rotation=0,transform=axlist[exp].transAxes,fontsize=14)
#     i=i+1
# #======================
# axlist[lexp[0]].set_ylabel("$\Delta$$AmpDiff$ (open-loop)", color='k',fontsize=10)
# im=plt.scatter([],[],c=[],cmap=cmap,s=0.1,vmin=vmin,vmax=vmax,norm=norm,zorder=101)
# im.set_visible(False)
# l,b,w,h = axlist[lexp[-1]].get_position().bounds
# cax=fig.add_axes([l+w*1.1,b+0.05*h,0.01,0.9*h])
# cbar=plt.colorbar(im,cax=cax,ticks=np.arange(vmin,vmax+0.1,0.2),orientation='vertical',extend="both")#np.arange(-0.5,0.5+0.001,0.25)
# cbar.ax.tick_params(labelsize=6.0)
# ##########
# plt.savefig("./pptimage/fig-scatter_NSEAI_All_WSE_Q_error.jpg")  # saves the current figure into a pdf page
# plt.close()