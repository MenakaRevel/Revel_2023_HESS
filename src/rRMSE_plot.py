...
experiments=["NOM_WSE_E2O_HWEB_004","NOM_WSE_E2O_HWEB_001","NOM_WSE_E2O_HWEB_002"] #
...
#----
rivnum=DA_dir+"/dat/rivnum_"+mapname+".bin"
...
def read_data(inputlist):
    ...
    # corrpted
    fname=DA_dir+"/out/"+experiment+"/assim_out/ens_xa/open/"+yyyy+mm+dd+"_"+numch+"_xa.bin"
    ...
    # assimilated
    fname=DA_dir+"/out/"+experiment+"/assim_out/ens_xa/assim/"+yyyy+mm+dd+"_"+numch+"_xa.bin"
...
def make_NSEAI():
    ...
    for exp in np.arange(len(experiments)):
        ...
        for point in np.arange(pnum):
            ...
            #
            RMSEasm=NS(np.mean(asm[:,:,point],axis=1),org)
            RMSEopn=NS(np.mean(opn[:,:,point],axis=1),org)
            rRMSE=(RMSEasm-RMSEopn)/(RMSEopn+1.0e-20)
            print (experiments[exp], pname[point], rRMSE)
            if obs==0.0:
                print ("no obs", obs)
                metric_frag.append(-9999.0)
            else:
                metric_frag.append(rRMSE)
...
labels=["Norm_Zero", "Norm_90MS","Norm_92MS"] #,"Norm_90",] #Restricted HydroWeb
fname="./data/RMSE_20210901.bin"
...
data0=metric[np.all(metric!=-9999.0,axis=1),:]
data=np.mean(data0,axis=1)
