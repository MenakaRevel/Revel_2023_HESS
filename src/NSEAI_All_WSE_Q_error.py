...
#============================
colors={lexp[0]:"#D81B60",lexp[1]:"#FFC107",lexp[2]:"#004D40"}
#============================
for exp in lexp:
    # discharge
    #----------
    ax1.plot(lbiasQ[exp],lNSEAI[exp],color=colors[exp],linestyle='None',linewidth=0,marker="o",fillstyle="none",markersize=5)
    slope, intercept, r, p, se = stats.linregress(lbiasQ[exp],lNSEAI[exp])
    ax1.plot(np.sort(lbiasQ[exp]),intercept + slope*np.sort(lbiasQ[exp]), color=colors[exp])
    print ('lbiasQ', exp, slope, intercept, r, p, se)
    #----------
    ax2.plot(lampdQ[exp],lNSEAI[exp],color=colors[exp],linestyle='None',linewidth=0,marker="o",fillstyle="none",markersize=5)
    slope, intercept, r, p, se = stats.linregress(lampdQ[exp],lNSEAI[exp])
    ax2.plot(np.sort(lampdQ[exp]),intercept + slope*np.sort(lampdQ[exp]), color=colors[exp])
    print ('lampdQ', exp, slope, intercept, r, p, se)
    # WSE
    #----------
    ax3.plot(lbiasW[exp],lNSEAI[exp],color=colors[exp],linestyle='None',linewidth=0,marker="o",fillstyle="none",markersize=5)
    slope, intercept, r, p, se = stats.linregress(lbiasW[exp],lNSEAI[exp])
    ax3.plot(np.sort(lbiasW[exp]),intercept + slope*np.sort(lbiasW[exp]), color=colors[exp])
    print ('lbiasW', exp, slope, intercept, r, p, se)
    #----------
    ax4.plot(lampdW[exp],lNSEAI[exp],color=colors[exp],linestyle='None',linewidth=0,marker="o",fillstyle="none",markersize=5)
    slope, intercept, r, p, se = stats.linregress(lampdW[exp],lNSEAI[exp])
    ax4.plot(np.sort(lampdW[exp]),intercept + slope*np.sort(lampdW[exp]), color=colors[exp])
    print ('lampdW', exp, slope, intercept, r, p, se)
#========================================================
ax1.text(-0.30,0.5,"Discharge",horizontalalignment='center',
        verticalalignment='center',rotation=90,transform=ax1.transAxes,fontsize=14)
ax1.text(0.5,1.2,"Bias",horizontalalignment='center',
        verticalalignment='center',rotation=0,transform=ax1.transAxes,fontsize=14)
ax1.set_xlabel("$pBias$ (open-loop)", color='k',fontsize=10)
ax1.set_ylabel("$NSEAI$", color='k',fontsize=10)
ax2.text(0.5,1.2,"Amplitude Diff",horizontalalignment='center',
        verticalalignment='center',rotation=0,transform=ax2.transAxes,fontsize=14)
ax2.set_xlabel("$rAmpDiff$ (open-loop)", color='k',fontsize=10)
ax2.set_ylabel("", color='k',fontsize=10)
ax3.text(-0.30,0.5,"WSE",horizontalalignment='center',
        verticalalignment='center',rotation=90,transform=ax3.transAxes,fontsize=14)
ax3.set_xlabel("$Bias$ (open-loop)", color='k',fontsize=10)
ax3.set_ylabel("$NSEAI$", color='k',fontsize=10)
ax4.set_xlabel("$\Delta$$AmpDiff$ (open-loop)", color='k',fontsize=10)
ax4.set_ylabel("", color='k',fontsize=10)
# plt.show()
##########
plt.savefig("./pptimage/fig-NSEAI_All_WSE_Q_error.jpg")  # saves the current figure into a pdf page
plt.close()
