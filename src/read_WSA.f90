program read_wsa
        ...
        do mon=smon, emon
            ...
            do day=1, eday
                ...
                do ens=1, ens_num
                    ...
                    ! courrpted
                    fname=trim(indir)//"/"//trim(expname)//"/assim_out/fldarea/open/fldarea"//trim(yyyymmdd)//"_"//numch//".bin"
                    ...
                    close(34)
                    ! assimilated
                    fname=trim(indir)//"/"//trim(expname)//"/assim_out/fldarea/assim/fldarea"//trim(yyyymmdd)//"_"//numch//".bin"
                    open(34,file=fname,form="unformatted",access="direct",recl=4*nXX*nYY,status="old",iostat=ios)
                    if(ios==0)then
    ...
    do i=1, N 
        ! open
        fout=trim(adjustl(outdir))//"/"//trim(expname)//"/wsa/opn_"//trim(adjustl(daylist(i)))//".bin"
        open(71,file=fout,form="unformatted",access="direct",recl=4*nXX*nYY,status="replace",iostat=ios)
        if(ios==0)then
            write(71,rec=1) opnout(i,:,:)
        else
            write(*,*) "no: ",trim(fname)
        end if
        close(71)
        ! assim
        fout=trim(adjustl(outdir))//"/"//trim(expname)//"/wsa/asm_"//trim(adjustl(daylist(i)))//".bin"
        open(72,file=fout,form="unformatted",access="direct",recl=4*nXX*nYY,status="replace",iostat=ios)
        if(ios==0)then
            write(72,rec=1) asmout(i,:,:)
        else
            write(*,*) "no: ",trim(fname)
        end if
        close(72)
...
!     fout=trim(adjustl(outdir))//"/"//trim(expname)//"/wse."//trim(obstype)//"/"//trim(adjustl(station))//".txt"
