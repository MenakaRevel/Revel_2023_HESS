...
#====================================================================
def plot_scatter(stat, lstato, color, numch, ylabel, xlabel,ax=None,left=False,bottom=False,sci=False):
    ax=ax or plt.gca()
    # ax2=ax2 or plt.gca()
    ax.plot(lstato, stat, color=color, linewidth=0.0,marker="o",markersize=5,fillstyle="none")
    ...
    ax.text(-0.10,1.05,"%s)"%(numch),ha="left",va="center",transform=ax.transAxes,fontsize=10)
    return 0
#====================================================================
...
def ret_metric(exp):
    ...
    return ldelr, lNSEAI, lriss, lbiaso, ldelAo, ldpeako
#==================
#------------------
# figure in A4 size
...
hgt=(11.69 - 2*va_margin)*(8.27/11.69)
...
for j,exp in enumerate(lexp):
    ldelr, lNSEAI, lriss, lbiaso, ldelAo, ldpeako=ret_metric(exp)
    # for BIAS
    plot_scatter(ldelr, lbiaso,colors[j],"a","$r$","$BIAS$",ax=ax1,left=True,bottom=False,sci=False)
    plot_scatter(lNSEAI, lbiaso, colors[j],"d","$NSE$","$BIAS$",ax=ax2,left=True,bottom=False,sci=False)
    plot_scatter(lriss, lbiaso, colors[j],"g","$ISS$","$BIAS$",ax=ax3,left=True,bottom=True,sci=True)
    # for delat A
    plot_scatter(ldelr, ldelAo,colors[j],"b","$r$","$\Delta$$A$",ax=ax4,left=False,bottom=False,sci=False)
    plot_scatter(lNSEAI, ldelAo, colors[j],"e","$NSE$","$\Delta$$A$",ax=ax5,left=False,bottom=False,sci=False)
    plot_scatter(lriss, ldelAo, colors[j],"h","$ISS$","$\Delta$$A$",ax=ax6,left=False,bottom=True,sci=True)
    # for delta t
    plot_scatter(ldelr, ldpeako,colors[j],"c","$r$","$\Delta$$t$",ax=ax7,left=False,bottom=False,sci=False)
    plot_scatter(lNSEAI, ldpeako, colors[j],"f","$NSE$","$\Delta$$t$",ax=ax8,left=False,bottom=False,sci=False)
    plot_scatter(lriss, ldpeako, colors[j],"i","$ISS$","$\Delta$$t$",ax=ax9,left=False,bottom=True,sci=True)
#==============================
patches=[]
