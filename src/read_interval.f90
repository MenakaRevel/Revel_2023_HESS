program read_int
    ...
    real,allocatable              ::  opn(:,:,:), opnout(:,:,:), opnmin(:,:,:), opnmax(:,:,:)
    real,allocatable              ::  asm(:,:,:), asmout(:,:,:), asmmin(:,:,:), asmmax(:,:,:)
    ...
    close(11)
    !==
22  format(a8,4x,f15.2,4x,f15.2,4x,f15.2,4x,f15.2,4x,f15.2,4x,f15.2)
    !==
    allocate(opn(nXX,nYY,ens_num),opnout(N,nXX,nYY),opnmin(N,nXX,nYY),opnmax(N,nXX,nYY))
    allocate(asm(nXX,nYY,ens_num),asmout(N,nXX,nYY),asmmin(N,nXX,nYY),asmmax(N,nXX,nYY),daylist(N))
    !================
    num=1
    do year=syear,eyear
        isleap=leap(year)
        do mon=smon, emon
            ...
            do day=1, eday
                ...
                opnmin(num,:,:)=minval(opn,DIM=3)
                opnmax(num,:,:)=maxval(opn,DIM=3)
                ! assimilated
                asmout(num,:,:)=sum(asm,DIM=3)/(real(ens_num)+1e-20)
                asmmin(num,:,:)=minval(asm,DIM=3)
                asmmax(num,:,:)=maxval(asm,DIM=3)
...
1000 continue
    ...
    fout=trim(adjustl(outdir))//"/"//trim(expname)//"/Q_interval/"//trim(adjustl(id))//".txt"
    ...
    do i=1, N 
        if (ix2==-9999 .or. iy2==-9999) then
            ...
            write(72,22) daylist(i), asmout(i,ix1,iy1), asmmin(i,ix1,iy1), asmmax(i,ix1,iy1), opnout(i,ix1,iy1), opnmin(i,ix1,iy1), opnmax(i,ix1,iy1)
        else
            ! print'(a8,4x,f10.2,4x,f10.2)', daylist(i), asmout(i,ix1,iy1)+asmout(i,ix2,iy2), opnout(i,ix1,iy1)+opnout(i,ix2,iy2)
            write(72,22) daylist(i), asmout(i,ix1,iy1)+asmout(i,ix2,iy2), asmmin(i,ix1,iy1)+asmmin(i,ix2,iy2), asmmax(i,ix1,iy1)+asmmax(i,ix2,iy2),opnout(i,ix1,iy1)+opnout(i,ix2,iy2), opnmin(i,ix1,iy1)+opnmin(i,ix2,iy2), opnmax(i,ix1,iy1)+opnmax(i,ix2,iy2)
        end if
    end do
    close(72)
    goto 1000
1090 continue
    close(11)
    deallocate(opn,opnout,opnmin,opnmax,asm,asmout,asmmin,asmmax,daylist)
end program read_int
!*********************************
function leap(year)
    implicit none
    integer                        :: year,leap
...
!*********************************
subroutine percentile(x,p,quart)
    implicit none
