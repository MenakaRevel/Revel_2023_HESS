...
def ret_metric(exp):
    ...
    return ldelr, lNSEAI, lriss, lPBo, lPBa
...
for j,exp in enumerate(lexp):
    ldelr, lNSEAI, lriss, lPBo, lPBa=ret_metric(exp)
    plot_scatter(ldelr, lPBo, lPBa,colors[j],"$\Delta$$r$",ax1=ax1,ax2=ax2)
    plot_scatter(lNSEAI, lPBo, lPBa,colors[j],"$NSEAI$",ax1=ax3,ax2=ax4)
    plot_scatter(lriss, lPBo, lPBa,colors[j],"$rISS$",ax1=ax5,ax2=ax6)
