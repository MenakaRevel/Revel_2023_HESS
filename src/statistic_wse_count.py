...
for i,exp in enumerate(experiments):
    ...
    for line in lines[1::]:
        ...
        if rRMSE <= 0.0:
            rRMSE_asm_count=rRMSE_asm_count+1
    ...
    for line in lines[1::]:
        ...
        rRMSE=(RMSEasm-RMSEopn)/(RMSEopn+1.0e-20)
        if rRMSE <= 0.0:
            rRMSE_val_count=rRMSE_val_count+1
        val_count=val_count+1
    metric_asm.append(rRMSE_asm_count)
    metric_val.append(rRMSE_val_count)
# np.array(metric)
print ("asm",np.shape(np.array(metric_asm)),asm_count)
print ("val",np.shape(np.array(metric_val)),val_count)
# print experiments
# print metric
df_asm=pd.DataFrame(data=metric_asm, columns=["rRMSE"], index=experiments)
print(df_asm)        
df_val=pd.DataFrame(data=metric_val, columns=["rRMSE"], index=experiments)
print(df_val)